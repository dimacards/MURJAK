...
- [x] **Этап 7** — редактирование товара через служебный чат. Клик «✏️ Редактировать» → бот ставит edit-lock в служебном чате + шлёт в ЛС работнику меню полей. После завершения — обновляются пост в канале и служебное сообщение. Поля: категория (inline), размер (inline), состояние/цена (текст), фото (полное пересоздание поста). MVP без лока на параллельное редактирование двумя работниками.
