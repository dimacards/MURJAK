  ...
  const setRes = await fetch(
    `https://api.telegram.org/bot${token}/setWebhook`,
    {
      ...
      body: JSON.stringify({
        ...
        // max_connections=1 — КРИТИЧНО для serverless с conversation-state в БД.
        // По умолчанию (40) Telegram шлёт апдейты параллельно. На альбоме из
        // 5-10 фото — N лямбд читают/пишут одну BotSession-запись одновременно,
        // race condition коррапит state, конверсейшн виснет, кнопки «Готово»
        // потом не работают (нет активной conversation). С max=1 апдейты идут
        // строго серийно — следующий уходит только после 200 на предыдущий.
        // Для одного работника пропускная способность не критична.
        // Webhook всё равно отвечает 200 на любую ошибку (см. /api/bot/route.ts),
        // так что очередь застрять не может.
        max_connections: 1,
      }),
    }
  );
