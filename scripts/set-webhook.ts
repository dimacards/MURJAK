  ...
  const setRes = await fetch(
    `https://api.telegram.org/bot${token}/setWebhook`,
    {
      ...
      body: JSON.stringify({
        ...
        // max_connections оставляем дефолтным (40). НЕ ставим 1 — это
        // сериализует доставку: если один апдейт обрабатывается долго или
        // падает с 500 (и ретраится), вся очередь — включая нажатия кнопок —
        // встаёт колом. Защита от дабл-тапа «Опубликовать» обеспечивается
        // тем, что после первого клика кнопки сразу убираются
        // (editMessageReplyMarkup), а ошибки гасит bot.catch (нет 500 → нет
        // ретраев).
      }),
    }
  );
